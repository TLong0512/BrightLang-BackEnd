...
public class UserRoadmapController : ControllerBase
{
    private readonly IUnitOfWork unitOfWork;
    private readonly IQuestionbankService questionbankService;
    private readonly ICurrentUserService currentUserService;

    public UserRoadmapController(IUnitOfWork unitOfWork, IQuestionbankService questionbankService, ICurrentUserService currentUserService)
    {
        ...
        [FromQuery] int page = 1, [FromQuery] int pageSize = 10)
    {
        ...
        foreach (UserRoadmap myRoadmap in myRoadmaps)
        {
            ...
            if (processes.Any() == true)
            {
                ...
                {
                    StartDate = processes.OrderBy(r => r.StartDate).First().StartDate,
                ...
                if (processStatistic.AllProcessCount == processStatistic.FinishedProcessCount)
                    processStatistic.EndDate = processes.OrderByDescending(r => r.EndDate).First().EndDate;
            }
    ...
    public async Task<ActionResult<UserRoadmapGeneralDto>> CreateMyRoadmap([FromBody] UserRoadmapPostDto userRoadmapPostDto)
    {
        ...
        return CreatedAtAction(nameof(GetMyRoadmap), new { id = myNewUserRoadmap.Id }, new UserRoadmapDetailDto
        {
            ...
            Processs = [], // no process was created.
        });
    }

    // UserRoadmap is not meant to be changed. if user want to enroll a new roadmap, 
